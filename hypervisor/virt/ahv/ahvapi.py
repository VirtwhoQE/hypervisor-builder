...
class AHVApi(object):
    ...
    def __init__(
        ...
        username,
        password,
        port=ahv_constants.DEFAULT_PORT,
        version=ahv_constants.VERSION_2,
        **kwargs,
    ):
        """
        Args:
    ...
    def _process_dict_response(self, data):
        ...
        if "resources" in formatted_data:
            if "power_state" in formatted_data["resources"]:
                formatted_data["power_state"] = formatted_data["resources"][
                    "power_state"
                ]
            if "num_cpu_sockets" in formatted_data["resources"]:
                formatted_data["num_cpu_sockets"] = formatted_data["resources"][
                    "num_cpu_sockets"
                ]
        ...
        for ii in range(retry_count):
            try:
                ...
                if self._internal_debug:
                    logger.debug(
                        f"{method.upper()} method The request url sent: {response.request.url}"
                    )
    ...
    def get_vm_by_name(
        self, guest_name, include_vm_disk_config=None, include_vm_nic_config=None
    ):
        """
