...
class PowerCLI:
    ...
    def _format(self, ret=0, stdout=None):
        ...
        """
        # regex = r"[[][.\s\S]*[]]"
        # res = re.findall(regex, stdout, re.DOTALL)
    ...
    def guest_del(self, guest_name):
        ...
        cmd = (
            f"{self.cert} Remove-VM -VM {guest_name} -DeletePermanently -Confirm:$false"
        )
        cmd += f'\''
        ret, _ = self.ssh.runcmd(cmd)
    ...
    def host_add(self, location, host_name, host_user, host_pwd):
        ...
        if self.host_exist(host_name):
            ...
            return True
        cmd = (
            f"{self.cert} Add-VMHost {host_name} "
            f"-Location {location} "
            f"-User {host_user} "
            f"-Password {host_pwd} "
            f"-confirm:$false"
