...
class PowerCLI:
    ...
    def host_search(self, host_name):
        ...
        host_msgs = {
            ...
            'host_data_store': output["StorageInfo"]["FileSystemVolumeInfo"].strip()
        }
        return host_msgs
    ...
    def host_set(self, host_name, state):
        ...
        :param state: Connected or Disconnected or Disconnected.
        :return:
        """
    ...
    def host_restart(self, host_name):
        ...
        :return:
        """
        cmd = f'{self.cert} Restart-VMHost {host_name} -confirm:$false'
        self.ssh.runcmd(cmd)
    ...
    def host_stop(self, host_name):
        ...
        cmd = f'{self.cert} Stop-VMHost {host_name} -confirm:$false'
        self.ssh.runcmd(cmd)
    ...
    def guest_del(self, guest_name):
        ...
        for i in range(5):
            ...
            ret, output = self.ssh.runcmd(cmd)
            if ret and self.guest_search(guest_name)['guest_state'] is not 0:
                logger.info("guest still power on, try again")
                self.guest_stop(guest_name)
            elif self.guest_exist(guest_name):
    ...
    def guest_start(self, guest_name):
        ...
        self.ssh.runcmd(cmd)
        for i in range(10):
            cmd = f'{self.cert} Get-VM {guest_name} | Get-VMQuestion | Set-VMQuestion ' \
                f'-Option "button.uuid.copiedTheVM" -Confirm:$false'
            self.ssh.runcmd(cmd)
            time.sleep(30)
            if self.guest_search(guest_name)['guest_state'] is 1:
                logger.info("Succeeded to start vcenter guest")
                guest_ip = self.guest_search(guest_name)['guest_ip']
                return guest_ip
            logger.warning("vcenter guest status is not PoweredOn, check again after 30s...")
    ...
    def guest_stop(self, guest_name):
        ...
        self.ssh.runcmd(cmd)
        for i in range(10):
            if self.guest_search(guest_name)['guest_state'] is 0:
    ...
    def guest_suspend(self, guest_name):
        ...
        self.ssh.runcmd(cmd)
        for i in range(10):
            if self.guest_search(guest_name)['guest_state'] is 2:
